...
function CodeEditor({ qu, qd, updateData, iTopic, iQu }) {
    ...
    try {
        ...
        const expectedOutput = resData.expected.replace(/\n/g, '<br>');
        const yourOutput = resData.got.output ? resData.got.output.replace(/\n/g, '<br>') : resData.got.error;

        // Compare outputs and set result class
        resData.got.output === resData.expected ? setResultClass("success") : setResultClass("failed");

        // Display the formatted result without spaces
        document.getElementsByClassName("result")[0].innerHTML = `
            <div style="white-space: pre-wrap;">Input:<br/>${resData.input}Expected output:<br/>${expectedOutput}Your output:<br/>${yourOutput}</div>
        `;
    } catch (error) {
